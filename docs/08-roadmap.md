...
> **Trạng thái:** cơ chế Phase 3 đã implement — `hatch run/plan/watch` + adapter headless (claude/codex/agy/kiro) + worktree + TUI `hatch board`. Việc còn lại: chạy song song thực sự nhiều agent (hiện `watch` tuần tự một pass) và kiểm thử với agent CLI cài đặt thật.
