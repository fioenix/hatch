...
Lifecycle trên là khung **mặc định**. Người dùng chọn một **template có sẵn** hoặc **viết workflow riêng** trong `.hatch/workflow.yaml`. Hatch ship các template phủ các phong cách product development lifecycle phổ biến:

| Template | Lanes | Hợp với |
|---|---|---|
| **`scrum`** (mặc định) | backlog → in-progress → review → done | sprint cố định + retro |
| **`kanban`** | + WIP limit | luồng liên tục, không sprint |
| **`spec-first`** | + spec | mọi epic qua requirements→design→tasks |
| **`lite`** | todo → doing → done | project nhỏ/cá nhân |
