...
type fakePost struct{ threadTS, username, icon, text string }
...
func (f *fakePoster) post(threadTS, username, icon, text string) (string, error) {
	f.posts = append(f.posts, fakePost{threadTS, username, icon, text})
	f.n++
...
func newTestBridge(t *testing.T, cfg Config) (*Bridge, *bus.Bus, *roster.Store, *fakePoster) {
	...
	rs := roster.New(l)
	fp := &fakePoster{}
	return NewBridge(b, rs, cfg, fp, loadThreadmap(l)), b, rs, fp
}
...
func TestMirrorOut(t *testing.T) {
	...
	if fp.posts[1].username != "claude" {
		t.Errorf("want impersonated username claude, got %q", fp.posts[1].username)
	}
}
